  ...
  try {
    ...
    try {
      const storeResponse = await fetch(
        `https://api.uploadcare.com/files/${arquivo.uuid}/storage/`,
        {
          method: 'PUT',
          headers: {
      ...
      if (!storeResponse.ok) {
        console.warn(
          '[UPLOAD] Aviso: O arquivo foi enviado, mas o armazenamento permanente não pôde ser confirmado.'
        );
      }
    } catch (storeError) {
...
export const apagaDoUploadCare = async (fileId) => {
  try {
    ...
    const response = await fetch(`https://api.uploadcare.com/files/${fileId}/storage/`, {
      method: 'DELETE',
      headers: {
        Authorization: `Uploadcare.Simple ${process.env.UPLOADCARE_PUBLIC_KEY}:${process.env.UPLOADCARE_SECRET_KEY}`,
